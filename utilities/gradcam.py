...
target_layers = [model.cls.layer1]

img_path = 'C:/wrd/DAGM/Class4/0022.PNG'
batch_size = 1
...
class GradCAM:
    ...
    def get_loss(self, output, target_category, mode='classification', label=None):
        loss = 0
        if mode == 'classification':
            ...
            if isinstance(output, tuple):
                output = output[0]
            for i in range(len(target_category)):
                loss = loss + self.criteon(output, label)
        return loss
...
x = valtrans(x).unsqueeze(0)
label = torch.tensor([1])
grayscale_cam = cam(input_tensor=x, target_category=target_category, mode='classification', label=label)
...
grayscale_cam.save(f'./gradcam_{img_path}')
# grayscale_cam *= 255
