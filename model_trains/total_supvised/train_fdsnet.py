        ...
        for i, (inputs_S1, labels_S1) in enumerate(self.train_loader):
            ...
            if isinstance(outputs, tuple):
                for index in range(len(outputs)):
                    ...
                    # 分类辅助损失
                    elif len(outputs[index].shape) == 2:
                        b, _, _ = labels_S1.shape
                        cls_label, _ = labels_S1.reshape(b, -1).max(dim=1)
                        loss_ce += self.cls_loss(outputs[index], cls_label.long())
            else:
                # CE_loss
