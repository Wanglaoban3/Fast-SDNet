...
class UAPSTrain(BaseNetWork):
    def __init__(self, epochs, benchmark, method, log_path, eta_consistency_weight):
        super().__init__(epochs, benchmark, method, log_path)
        self.kl_distance = nn.KLDivLoss(reduction='none')  # KL_loss for consistency training
        self.log_sm = torch.nn.LogSoftmax(dim=1)  # For computing the KL distance
    ...
    def epoch_train(self):
        ...
        for i, (inputs_U, labels_U) in enumerate(self.unlabeled_loader):  # (zip(train_loader, unlabeled_train_loader)):
            ...
            # unsupervised loss
            variance_main = torch.sum(self.kl_distance(self.log_sm(un_outputs), preds), dim=1)
            exp_variance_main = torch.exp(-variance_main)

            variance_aux1 = torch.sum(self.kl_distance(self.log_sm(un_outputs_aux1), preds), dim=1)
            exp_variance_aux1 = torch.exp(-variance_aux1)

            variance_aux2 = torch.sum(self.kl_distance(self.log_sm(un_outputs_aux2), preds), dim=1)
            exp_variance_aux2 = torch.exp(-variance_aux2)

            variance_aux3 = torch.sum(self.kl_distance(self.log_sm(un_outputs_aux3), preds), dim=1)
            ...
            ps_main_loss = 0.5 * (
                        self.ce_loss(un_outputs, un_lbl_pseudo) + self.dice_loss(un_lbl_pseudo.unsqueeze(1), un_outputs))
            ps_1_loss = 0.5 * (self.ce_loss(un_outputs_aux1, un_lbl_pseudo) + self.dice_loss(un_lbl_pseudo.unsqueeze(1),
                                                                                   un_outputs_aux1))
            ps_2_loss = 0.5 * (self.ce_loss(un_outputs_aux2, un_lbl_pseudo) + self.dice_loss(un_lbl_pseudo.unsqueeze(1),
                                                                                   un_outputs_aux2))
            ps_3_loss = 0.5 * (self.ce_loss(un_outputs_aux3, un_lbl_pseudo) + self.dice_loss(un_lbl_pseudo.unsqueeze(1),
                                                                                   un_outputs_aux3))
            # TAKING AVERAGE UNCERTAINTY
            ps_main_loss_un = torch.mean(ps_main_loss * exp_variance_main)
            ps_1_loss_un = torch.mean(ps_1_loss * exp_variance_aux1)
            ps_2_loss_un = torch.mean(ps_2_loss * exp_variance_aux2)
            ps_3_loss_un = torch.mean(ps_3_loss * exp_variance_aux3)
            ps_loss = (ps_main_loss_un + ps_1_loss_un + ps_2_loss_un + ps_3_loss_un) / 4
            ...
            # Total loss
            loss = supervised_loss + consistency_weight * ps_loss + consistency_weight * l_uncert
            loss.backward()
        ...
        self.writer.add_scalar('Train/mdloss', epoch_md_loss, self.epoch)
        ...
        self.writer.add_scalar('Train/PS-Loss', epoch_ps_loss, self.epoch)
        self.logger.info('Train uncertainty: {}'.format(epoch_uncertainity_loss))
        self.writer.add_scalar('Train/Uncertainty', epoch_uncertainity_loss, self.epoch)
