    ...
    def epoch_train(self):
        ...
        for i, (inputs_U, labels_U) in enumerate(self.unlabeled_loader):  # (zip(train_loader, unlabeled_train_loader)):
            if i >= iter_per_epoch:
                break
            try:
                inputs_lbl, labels_lbl = next(labeled_dataloader_iterator)
            except StopIteration:
                labeled_dataloader_iterator = iter(self.train_loader)
                inputs_lbl, labels_lbl = next(labeled_dataloader_iterator)

            inputs_l, labels_l = self.generate_mix_data(inputs_lbl, labels_lbl)
            new_inputs_U1, new_inputs_U2 = self.generate_crossmix_data(inputs_l, inputs_U1,
                                                                  inputs_U2)  # Cut and mix labeled with the unlabeled
